...
export class AddTaskComponent implements OnInit {
  ...
  constructor(private jsonService:JsongeneratorService, private toasterService:ToasterService , private titleService:Title) { 
...
addTask()
{
  ...
  {
    indicator = false;
    this.toasterService.deleteMessages("Invalid Start Date: Provide the start date in MM/dd/YYYY format");
  }
  else if(!data.startDate || data.endDate == null || data.endDate == "")
  {
    indicator = false;
    this.toasterService.deleteMessages("Invalid End Date: Provide the end date in MM/dd/YYYY format");
  }
  ...
  var taskNew = new Task(data.taskId, data.task, data.priority, "Started" , this.userAssociated, this.parentTaskAssociated, this.projectAssociated,this.convertDateFormat(data.startDate),this.convertDateFormat(data.endDate));
  this.resetForm();
...
selectManager(data:any)
{
  ...
  this.toasterService.infoMessages("Manager: " + data.employeeId  + " added successfully!! Kindly close the popup to proceed with Task creation");
}
...
selectTask(data:any)
{
  ...
  this.toasterService.infoMessages("Parent Task : " +data.parentTask  + " added successfully!! Kindly close the popup to proceed with Task creation");
}
...
enableDateField()
{
  ...
  { 
    ...
    convertDateFormat(dateproject:Date){

    var month:any =  dateproject.getMonth() + 1;
    var date:any = dateproject.getDate();
    if(month < 10)
      month = '0' + month;
    if(date < 10)
      date = '0' + date;

    var tommorowString :any = dateproject.getFullYear() + '-' + month + '-' + date;
    return tommorowString;
    }    
