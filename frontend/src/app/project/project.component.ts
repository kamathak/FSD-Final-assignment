...
export class ProjectComponent implements OnInit {
  ...
  { 
      ...
      (error)=>{
                 if(error.status = 500)
                 this.toasterService.deleteMessages("Project is currently associated with a task. Hence cannot be deleted");
                  else
...
updateProject(){
  ...
  if(data.endDate <data.startDate)
      this.toasterService.deleteMessages("Process Aborted !!! End Date is before Start Date");
  else{
      var projectNew = new Project(data.projectId, data.project,null, this.userAssociated, this.convertDateFormat(data.startDate), this.convertDateFormat(data.endDate),data.priority);
      this.resetForm();
  ...
  addProject()
  {
      ...
      if(data.endDate < data.startDate)
            this.toasterService.deleteMessages("Process Aborted !!! End Date is before the Start Date");
      else{
            var projectNew = new Project(data.projectId, data.project,null, this.userAssociated, this.convertDateFormat(data.startDate), this.convertDateFormat(data.endDate),data.priority);
            this.resetForm();
  ...
  selectManager(data:any)
  {
      ...
      this.toasterService.infoMessages("Manager: " + data.employeeId  + " added successfully!! Kindly close the popup to proceed with project creation");
  }
